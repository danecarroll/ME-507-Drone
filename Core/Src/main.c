...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define PID_SCALE 1000 //allow us to use ints, not floats for Kp of 1.1 --> 1100
#define true 1;
#define false 0;
#define BT_MSG_LEN 16
...
//State
int16_t roll_set, pitch_set, yaw_set, effort_set;                // from user control
int16_t roll_true, pitch_true, yaw_true;             // from IMU
int16_t roll_effort, pitch_effort, yaw_effort;       // output of control loop

// PID vars
int16_t roll_error, pitch_error, yaw_error;
int16_t roll_integral, pitch_integral, yaw_integral;
int16_t roll_derivative, pitch_derivative, yaw_derivative;
int16_t last_roll_error, last_pitch_error, last_yaw_error;        // error for sample n-1
int16_t last_last_roll_error, last_last_pitch_error, last_last_yaw_error; // error for sample n-2

// Gains
// Roll Axis
int16_t Kp_roll  = 1000;
int16_t Ki_roll = 0;
int16_t Kd_roll = 0;
// Pitch Axis
int16_t Kp_pitch  = 1000;
int16_t Ki_pitch = 0;
int16_t Kd_pitch = 0;
...
//BT
//uint8_t BT_ByteReceived = 0;
//uint8_t BT_BufferReceived[100];
//uint8_t BT_UART_Index = 0;
...
int main(void)
{
  ...
  MX_I2C3_Init();
  /* USER CODE BEGIN 2 */
  //HAL_UART_Receive_IT(&huart2, &BT_ByteReceived, 1); (old, for interupt based, CPU involved UART recpetion)
  ...
  { //potential to use HAL tim int flag if control loop needs to run at percise intervals to avoid spike in derivative/integral calculation
	 if (state == 0){
		 BNO_Init();
		 InitializeBT();
		 ////Initialize. Maybe move this to user init section so not running when in flight mode
		 //Arm ESC's
		 state = 1;
	 }else if(state == 1){
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        {
        	BT_BufferReceived[BT_UART_Index] = '\0';  // Null terminate
        	BT_UART_Index = 0;



            //HAL_UART_Transmit(&huart1, (uint8_t *)BT_BufferReceived, strlen((char *)BT_BufferReceived), HAL_MAX_DELAY); // Echo back
            processInput((char *)BT_BufferReceived, &roll_set, &pitch_set, &yaw_set, &effort_set);
        }
        else
        {
            if (BT_UART_Index < sizeof(BT_BufferReceived) - 1)
            {
            	 BT_BufferReceived[BT_UART_Index++] = BT_ByteReceived;
            }
        }

        // Re-arm the interrupt for next byte
        HAL_UART_Receive_IT(&huart2, &BT_ByteReceived, 1);
    }
}
*/
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
